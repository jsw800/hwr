...
class HwDataset(Dataset):
    ...
    def __len__(self):
        return len(self.data) * 9   # 9 fields per csv line

    def __getitem__(self, idx):
        item = self.data[idx / 9]
        item_idx = idx % 9
        ...
        coords = [int(coord) for coord in item[(5*item_idx + 2):(5* item_idx + 6)]]
        img = self.cur_img[coords[2]:coords[3], coords[0]:coords[1]]
        gt = item[5*item_idx + 1]
        ...
        if img.shape[0] == 0 or img.shape[1] == 0:
            print("Warning: image has no size:", os.path.join(self.root_path, item[0]))
            return None
